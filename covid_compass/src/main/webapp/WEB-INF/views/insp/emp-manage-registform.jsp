...
<body>
	...
	<form id="mberRegist" action="mberRegist" method="POST">
	    ...
	    <input type="hidden" id="instNo" name="instNo" value="${insp.instNo }" />
	...
	var checkEmail = false;
		...
		function mberRegist() {
			...
			if (result) {
				...
				$.ajax({
					url : '<%=request.getContextPath()%>/insp/mberRegist',
					type : 'post',
					data : { 'mberId' : mberId
						,'mberPwd' : mberPwd
						,'mberNm' : mberNm
		...
		function idcheck(){
			...
			$.ajax({
				type : 'POST',
				url : '<%=request.getContextPath()%>/rest-insp/idcheck',
				data : {'mberId':mberId},
