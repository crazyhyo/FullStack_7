...
<body>
		...
		<div class="row" style="margin-bottom: 7px;">
			...
			<div class="col-sm-3">
				<input name="mberTelno" type="text" class="form-control"
					id="mber_telno" value="" onkeyup="telCheck();" oninput="autoHyphen(this);">
	...
	<form id="mberRegist" action="mberRegist" method="POST">
	    ...
	    <input type="hidden" id="instNo" name="instNo" value="${ltct.instNo }" />
	...
	var checkEmail = false;
		...
		function mberRegist() {
			...
			if (result) {
				...
				$.ajax({
					url : '<%=request.getContextPath()%>/ltct/mberRegist',
					type : 'post',
					data : { 'mberId' : mberId
						,'mberPwd' : mberPwd
						,'mberNm' : mberNm
		...
		function idcheck(){
			...
			$.ajax({
				type : 'POST',
				url : '<%=request.getContextPath()%>/rest-ltct/idcheck',
				data : {'mberId':mberId},
