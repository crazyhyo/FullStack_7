...
<section class="content">
				...
				<div id="placeSearch" class="col-md-7" style="visibility: hidden; height: 30px;">
							<div class="input-group float-left" style="width: 98%;">
								...
								<input id="keywordId" class="form-control" type="text" name="keyword" value="${pageMaker.cri.keyword }" style="width: 18%; display: inline-block;" /> <span class="input-group-append">
									...
									onclick="list_go('<%=request.getContextPath()%>/rest-hspt/ltct-list-to-isoptnt', 1, '#ltct-all-list-table-tbody', '#inptnt-all-pagination-template' );" 
									data-url="<%=request.getContextPath()%>/rest-hspt/ltct-list-to-isoptnt">
	...
	<div class="col-lg-6 pr-0">	
		<div class="card" style="margin-top: 1px; width: 98%;">
			...
			<div class="card-body pb-0"  id="mapLocation">
				...
				<div style="float: right; margin-left: 15px; height: 15px;">
					<button id="bookmark-btn" type="button" id="bookmark-btn" data-ltct-no="" onclick="bookmark_go(this);" class="btn btn-primary btn-xs" style="display: none;">연계기관등록</button>
				</div>
				<table class="table table-bordered">
					<tbody> 
						<tr>
			...
			<div class="row" style="height: 38px;padding-right: 2.8%;">
					...
					<div class="col-md-4">
						<button type="button" style="background-color: #1a4f72; border-color: #1a4f72;"id="go-isoptnt-btn" data-ltct-no="" data-manage-no="${manageNo }"
                                class="btn btn-block btn-primary" onclick="process_go('${type}');">입소 신청</button>
					</div>
					<div class="col-md-4">
						<button type="button" onclick="closeCheck();" class="btn btn-block btn-danger ">닫기</button>
					</div>
				</div>
		</div>
	</div>
	...
	<script>
	function closeCheck(){
		var check = confirm("요청페이지를 닫으시겠습니까?");
		if(check){
			window.close();
		}else{
			return;
	...
	function bookmark_go(ltctNo, addr){
	...
	function bookmark_remove(ltctNo, addr){
		
		var check = confirm("연기기관을 해제하시겠습니까?");
		
		if(check){
			$.ajax({
			    url : '<%=request.getContextPath() %>/rest-hspt/bookmark-remove',
			    type : 'post',
			    dataType : 'json',
			    data : {'toInstNo' :  ltctNo},
			    success : function(dataMap){
	...
	function marker_check(opRate){
	   ...
	   return markerImage;
		...
		function makeSearch() {
      	...
      	function mapOpen(addr, instNm, rmndSckbdCnt, opRate, checkResult, ltctNo){
      		
      	  $('#map').remove();
      	  var html = "<div id='map' style='width: 94.3%; height: 430px; margin-top: 7px; margin-left: 17px; margin-bottom: 7px;'></div>";
      								...
      								if (status === kakao.maps.services.Status.OK) {
    									...
    									if(checkResult > 0){
    										iwContent = '<div class="markerInfo" style="width:170px;text-align:left;padding-bottom: 0;padding-top: 3px;display:block; ">'
    											...
    											+ opRate
    											+ "%</span>"
    											+ '<br> <button class="btn btn-block btn-secondary btn-xs" onclick="bookmark_remove(\''
    											+ ltctNo
    											+ '\',\''
    											+ addr
    											+ '\');" type="button" style="width: 100%;background:#1a4f72;border-color:#1a4f72;">연계기관해제'
    											+ '</buttom></div>';
    									}
    									else{
    										iwContent = '<div class="markerInfo" style="width:170px;text-align:left;padding-bottom: 0;padding-top: 3px;display:block; ">'
    											...
    											+ opRate
    											+ "%</span>"
    											+ '<br> <button class="btn btn-block btn-secondary btn-xs" onclick="bookmark_go(\''
    											+ ltctNo
    											+ '\',\''
    											+ addr    											
    											+ '\');" type="button" style="width: 100%;background:#1a4f72;border-color:#1a4f72;">연계기관등록'
    											+ '</buttom></div>';
    									}
      									// 인포윈도우를 생성합니다
...
<script>
                   ...
                   function initKakaoMap(ltctList, adres){
                   	  $('#map').remove();
                   	  var html = "<div id='map' style='width: 94.3%; height: 430px; margin-top: 7px; margin-left: 17px; margin-bottom: 7px;'></div>";
                  	  $('#mapLocation').after(html);                    	   
