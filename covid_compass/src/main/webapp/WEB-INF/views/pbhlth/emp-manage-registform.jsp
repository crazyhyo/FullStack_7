...
<body>
	...
	<form id="mberRegist">
	    ...
	    <input type="hidden" id="instNo" name="instNo" value="${pbht.instNo }" />
	...
	var checkEmail = false;
		...
		function mberRegist() {
			...
			if (result) {
				...
				$.ajax({
					url : '<%=request.getContextPath()%>/rest-pbht/mberRegist',
					type : 'post',
					data : { 'mberId' : mberId
						,'mberPwd' : mberPwd
						,'mberNm' : mberNm
		...
		function idcheck(){
			...
			$.ajax({
				type : 'POST',
				url : '<%=request.getContextPath()%>/rest-pbht/idcheck',
				data : {'mberId':mberId},
