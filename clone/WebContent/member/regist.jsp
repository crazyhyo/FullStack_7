...
function regist_go(){
	...
	//비밀번호는 20글자 영문자 숫자 특수문자 조합
	var reqPWD=/^[a-zA-Z0-9!@#\$%\^&\*]{1}[a-zA-Z0-9!@#\$%\^&\*]{5,19}/;
	if(!reqPWD.test(input_PWD.val())){
		alert("비밀번호는 6~20자의 영문자, 숫자, 특수문자로 입력해야 합니다.")
		input_PWD.focus();
		return;
	}
	...
	//이름은는 20글자 영문자 숫자 특수문자 조합
	var reqNAME=/^[가-힣a-zA-Z]{2,20}$/;
	if(!reqNAME.test(input_NAME.val())){
		alert("이름은 한글 및 영문자로 2~20글자로 입력해야 합니다.")
		input_NAME.focus();
		return;
	}
	...
	//이메일
	var reqEMAIL=/^[a-z0-9\.\-_]+@([a-z0-9\-]+\.)+[a-z]{2,6}$/;
	if(!reqEMAIL.test(input_EMAIL.val())){
		alert("이메일 형식에 맞지 않습니다.")
		input_EMAIL.focus();
		return;
	}
	
	if(!checkedID || checkedID != input_ID.val().trim()){
		alert("중복확인을 해주세요");
		return;
	}
