		          ...
		          <div class="card-footer" style="padding:5px 0;" >
		          	...
		          	<%-- 	<c:if test="${loginUser.id eq member.id }"> --%>
			          		<div class="col-sm-3 text-center">
			          			<button type="button" onclick="location.href='modify?id=${member.id}';" 
			          					<%-- style="display:${loginUser.id eq member.id ? 'visible':'none'} ;" --%> id="modifyBtn" class="btn btn-warning ">수 정</button>
			          		</div>
		          		
			          		<div class="col-sm-3 text-center">
			          			<button type="button" onclick="location.href='remove?id=${member.id}';" 
			          			id="deleteBtn" class="btn btn-danger" >삭 제</button>
			          		</div>
		          			
			          		<div class="col-sm-3 text-center">
			          			<c:if test="${member.enabled ne 0}">
				          			<button type="button" onclick="location.href='enabled?id=${member.id}&enabled=0';" id="stopBtn" class="btn btn-info" >비활성</button>
			          			</c:if>
			          			<c:if test="${member.enabled eq 0}">
			          				<button type="button" onclick="location.href='enabled?id=${member.id}&enabled=1';" id="activeBtn" class="btn btn-info" >활&nbsp;&nbsp;성</button>
			          			</c:if>
			          		</div>
		          		<%-- </c:if> --%>
...
function modify_go(id){
	location.href = "<%=request.getContextPath()%>/member/modify?id=" + id; 
}
